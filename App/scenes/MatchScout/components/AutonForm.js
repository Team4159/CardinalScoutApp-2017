...
const AutonForm = ({ onPlusPress, onMinusPress, push, submit, data }) =>{
  return(
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        <SmallButton text='T'style={data.cross==='T' ? {backgroundColor: 'gray'}:styles.counterButton } onPress={() => submit({cross: 'T'})} disabled={data.cross==='T'}/>
        <SmallButton text='F' style={data.cross==='F' ? {backgroundColor: 'gray'}: styles.counterButton} onPress={() => submit({cross: 'F'})} disabled={data.cross==='F'}/>
      </View>
